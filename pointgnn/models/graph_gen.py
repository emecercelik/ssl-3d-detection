...
def gen_multi_level_local_graph_v3(
    ...
    for config in level_configs:
        ...
        center_xyz = vertex_coord_list[graph_level+1] # Keypoints in each of the voxels
        vertices = gen_graph_fn(points_xyz, center_xyz, **method_kwarg)
        edges_list.append(vertices) # List of connection pairs [center_i,point_j]
...
# radius (random?) nearest neighbours
def gen_disjointed_rnn_local_graph_v3(
    ...
    # Returns indices of neighbor points of each of center_xyz
    # len(indices)= nc
    ...
    vertices_v = np.concatenate(indices)#Indices of connected neighbors
    # Indices of centers 
    vertices_i = np.concatenate(
        [i*np.ones(neighbors.size, dtype=np.int32)
            for i, neighbors in enumerate(indices)])
    # Connection indices: From center to points 
    # [[452,0],[312,0],...,[412,3],[5,3],...,[678,nc-1]]
    vertices = np.array([vertices_v, vertices_i]).transpose()
    '''
    print('------------------------------------')
    print('Scale:', scale)
    print('num points: ', np.shape(points_xyz))
    print('point example:', points_xyz[:2])
    print('num_centers:', np.shape(center_xyz))
    print('center example:', center_xyz[:2])
    print('nbrs:', dir(nbrs))
    print('num indices:', np.shape(indices))
    print('indices ex:', indices[:2])
    print('shape vertices_v:', np.shape(vertices_v))
    print('shape vertices_i:', np.shape(vertices_i))
    print('shape vertices:', np.shape(vertices))
    print('ex vertices_v:', vertices_v[:2])
    print('ex vertices_i:', vertices_i[:2])
    print('ex vertices:', vertices[:2])
    print('------------------------------------')
    '''
    return vertices
