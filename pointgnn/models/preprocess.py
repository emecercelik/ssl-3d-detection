...
def random_jitter(cam_rgb_points, labels, xyz_std=(0.1, 0.1, 0.1)):
    ...
    random_values = {'x_delta':x_delta,'y_delta':y_delta,'z_delta':z_delta}
    return Points(xyz=xyz, attr=cam_rgb_points.attr), labels, random_values
...
def random_drop(cam_rgb_points, labels, drop_prob=0.5, tier_prob=None):
    ...
    random_values['drop_prob'] = drop_prob
    xyz = cam_rgb_points.xyz
    mask = np.random.uniform(size=xyz.shape[0]) > drop_prob
    if np.sum(mask) == 0:
        # print("Warning: attempt to drop all points, restore to all points")
        mask = np.ones_like(mask)
    random_values['mask']=mask
    return Points(xyz=xyz[mask], attr=cam_rgb_points.attr[mask]), labels,random_values
...
def random_rotation_all(cam_rgb_points, labels, method_name='normal',
    ...
    random_values={'delta_yaw':delta_yaw}
    return Points(xyz=xyz, attr=cam_rgb_points.attr), labels, random_values
...
def random_flip_all(cam_rgb_points, labels, flip_prob=0.5):
    ...
    random_values={'p':p,'flip_prob':flip_prob}
    return Points(xyz=xyz, attr=cam_rgb_points.attr), labels, random_values
...
def random_box_rotation(cam_rgb_points, labels, max_overlap_num_allowed=0.1,
    ...
    new_labels = []
    random_values={}
    random_values['delta_yaw']=[]
    random_values['labels']=[]
        ...
        if label['name'] in augment_list:
            ...
            for trial in range(max_trails):
                ...
                if np.sum(more_mask) < max_overlap_num_allowed:
                    ...
                    random_values['delta_yaw'].append(delta_yaw)
                    random_values['labels'].append(label)
                    sucess = True
                    break;
            if not sucess:
    ...
    return Points(xyz=xyz, attr=cam_rgb_points.attr), new_labels,random_values
...
def random_box_global_rotation(cam_rgb_points, labels,
    ...
    new_labels.extend([l for l in labels if l['name'] == 'DontCare'])
    assert len(new_labels) == len(labels)
    return Points(xyz=xyz, attr=attr), new_labels, random_values
...
def random_box_shift(cam_rgb_points, labels, max_overlap_num_allowed=0.1,
    ...
    random_values= {}
    random_values['delta_x']=[]
    random_values['delta_y']=[]
    random_values['delta_z']=[]
    random_values['labels']=[]
    for i, label in enumerate(labels_no_dontcare):
        if label['name'] in augment_list:
            trial = 0
            ...
            for trial in range(max_trails):
                ...
                if below_overlap:
                    ...
                    # update boxes and label
                    new_labels.append(new_label)
                    random_values['delta_x'].append(delta_x)
                    random_values['delta_y'].append(delta_y)
                    random_values['delta_z'].append(delta_z)
                    random_values['labels'].append(label)
                    sucess = True
                    break;
            if not sucess:
...
def dilute_background(cam_rgb_points, labels, dilute_voxel_base=0.4,
    ...
    random_values = {'voxel_size':dilute_voxel_base}
    return Points(
        xyz=np.concatenate([front_xyz, diluted_background_points.xyz], axis=0),
        attr=np.concatenate([front_attr,
            diluted_background_points.attr], axis=0)), labels_no_dontcare,random_values
...
def remove_background(cam_rgb_points, labels, expend_factor=(4.0, 4.0, 4.0),
    ...
    return Points(xyz=xyz[mask],
        attr=cam_rgb_points.attr[mask]), labels_no_dontcare, random_values
...
def random_transition(cam_rgb_points, labels, xyz_std=(0.1, 0.1, 0.1)):
    ...
    for label in labels:
        ...
        label['y3d'] += y_delta
        label['z3d'] += z_delta
    random_values['x_delta'] = x_delta
    random_values['y_delta'] = y_delta
    random_values['z_delta'] = z_delta
    return Points(xyz=xyz, attr=cam_rgb_points.attr), labels, random_values
...
def get_data_aug(aug_configs=[]):
    ...
    def multiple_aug(cam_rgb_points, labels):
        ...
        for aug_config in aug_configs:
            ...
            cam_rgb_points, labels,aug_values = aug_method(
                cam_rgb_points, labels, **aug_config['method_kwargs'])
            # TODO: Return the random values (aug_random_val_dict) to be applied
            # to the previous frame as well, if necessary
            aug_random_val_dict[aug_config['method_name']] = aug_values
        return cam_rgb_points, labels
    return multiple_aug
