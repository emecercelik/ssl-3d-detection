...
from mmdet3d.models.losses.cycle_loss import Cycle_Loss
...
# extract points features
    # extract_pts_feat for flow 
    def extract_pts_feat_flow(self, pts, img_feats, img_metas):
        """Extract features of points."""
        # Because we don't need 3d-detection-head at the first step, we just comment these followed two rows.
        # if not self.with_pts_bbox:
        #     return None
        voxels, num_points, coors = self.voxelize(pts)

        # we return "pred_f, second_sample_point_coors, pred_b, first_sample_point_coors" from voxel-encoder, so that we can do the backpropagation and calculate cycel_loss
        voxel_features, pred_f, p2_hatnn_coors, pred_b, first_sample_point_coors = self.pts_voxel_encoder(voxels, num_points, coors,
                                                img_feats, img_metas)
        ...
        if self.with_pts_neck:
            x = self.pts_neck(x)
        return x, pred_f, p2_hatnn_coors, pred_b, first_sample_point_coors
    
    # extract_pts_feat for baseline and det_head
    ...
    def extract_feat(self, points, img, img_metas):
        ...
        img_feats = self.extract_img_feat(img, img_metas) # we don't use images, so just ignore this img_feats
        if self.flow:
            pts_feats, pred_f,  p2_hatnn_coors, pred_b, first_sample_point_coors = self.extract_pts_feat_flow(points, img_feats, img_metas)            
            return (img_feats, pts_feats, pred_f, p2_hatnn_coors, pred_b, first_sample_point_coors)
...
# forward step of training
    ...
    # get all features
        losses = dict()
        f_cycle_loss = Cycle_Loss()
        
        if self.flow:
            img_feats, pts_feats, pred_f,  p2_hatnn_coors, pred_b, first_sample_point_coors = self.extract_feat(
                points, img=img, img_metas=img_metas)
             # calculate cycle_loss
            cycle_loss = f_cycle_loss(pred_f, 
                                     p2_hatnn_coors, 
                                     pred_b, 
                                     first_sample_point_coors)
        ...
        return losses
# because we don't use 3d detection head, just ignore this function
